...
fires.focal.names = "1994COTTONWOOD"
...
filtered.fire.sev.backup = fire.sev

#### correct the severity layer
highsev_override = st_read("data/non-synced/existing-datasets/VegBurnSeverity_shp/veg_burn_severity_cottonwood_highOverride.shp",stringsAsFactors=FALSE)
lowsev_override = st_read("data/non-synced/existing-datasets/VegBurnSeverity_shp/veg_burn_severity_cottonwood_lowOverride.shp",stringsAsFactors=FALSE)

highsev_override = st_union(highsev_override) %>% st_sf()
lowsev_override = st_union(lowsev_override) %>% st_sf()

highsev_override = st_transform(highsev_override,st_crs(fire.sev)) %>% st_buffer(0)
lowsev_override = st_transform(lowsev_override,st_crs(fire.sev)) %>% st_buffer(0)
fire.sev = fire.sev %>% st_buffer(0)

### need to cut up the fire sev raster

# turn highsev into the cutting lines
highsev.lines = st_cast(highsev_override,"MULTILINESTRING")
highsev_cut = st_buffer(highsev.lines,0.5)

lowsev.lines = st_cast(lowsev_override,"MULTILINESTRING")
lowsev_cut = st_buffer(lowsev.lines,0.5)

all_cut = st_union(highsev_cut,lowsev_cut)

fire.sev2 = st_difference(fire.sev,all_cut)
fire.sev3 = st_cast(fire.sev2, "POLYGON")

# which of the fire sev slices are overlaid by a high sev override?
a = st_intersects(fire.sev3,highsev_override,sparse=FALSE)
fire.sev3$high_override_tf = a

# which of the fire sev slices are overlaid by a low sev override?
a = st_intersects(fire.sev3,lowsev_override,sparse=FALSE)
fire.sev3$low_override_tf = a


fire.sev3$BURNSEV = ifelse(fire.sev3$high_override_tf,4,fire.sev3$BURNSEV)
fire.sev3$BURNSEV = ifelse(fire.sev3$low_override_tf,1,fire.sev3$BURNSEV)


st_write(fire.sev3,"data/non-synced/existing-datasets/VegBurnSeverity_shp/veg_burn_severity_cottonwood_corrected.gpkg")
...
candidate.plots <- candidate.plots %>%
  ...
  filter(dist.non.high > 2) # all high severity in and surrounding th eplot
...
for(i in 1:nrow(trt)) {
  ...
  ctl.close <- ctl.close[ctl.close$most.recent.focal.fire == trt.focal$most.recent.focal.fire &
                           ...
                           near(ctl.close$dist.non.high,trt.focal$dist.non.high,70) &
...
candidate.plots.paired[candidate.plots.paired$dist.non.high < 100,"dist.nonhigh"] <- "< 80 m"
...
### remove plots from inaccessible areas
inaccessible = st_read("data/site-selection/input/cottonwood_too_far.shp")
inaccessible = st_transform(inaccessible,st_crs(p))
p.over = st_intersects(p,inaccessible,sparse = FALSE) %>%rowSums()
p = p[!p.over,]
...
p.dat.agg.many <- p.dat.agg[p.dat.agg$nplots >= 0,] %>% 
  as.data.frame()
...
st_write(p.dat.many.w.ctl,"data/site-selection/output/candidate-plots/candidate_plots_paired_filtered_moontelope_45m-45m_cottonwood_nofilter3.gpkg",delete_dsn=TRUE)
...
pdf("data/site-selection/output/candidate-plots/stratification_v24_cottonwood_45m-45m_v2.pdf")
...
### create a column for plotting the different treatment types based on color



#Targets are

##!!!!!make sure to remove perimeter plots far from seed source

a = "salv: neither, prp: no, rel: el, thn: no, replt: YES"
b = "salv: neither, prp: no, rel: em, thn: no, replt: YES" # maybe add to above if not concerned about the different release schedule; but the one above already has enough


c = "salv: neither, prp: no, rel: e, thn: no, replt: YES" ## only interested in years 3-4, and make sure to drop perimeter that are far from seed source  ## compare year 4 perimeter and internal>

## Completely unmanaged:
# internal, yr 3 only:
d = "salv: neither, prp: no, rel: no, thn: no, replt: no"

#perimeter, yrs 2 and 3
e = "salv: neither, prp: no, rel: no, thn: no, replt: no"

# could mix those two above with replanted ones:
# perimeter and internal
f = "salv: neither, prp: no, rel: no, thn: no, replt: YES"


## outlier of salvaged
g = "salv: planted, prp: no, rel: e, thn: no, replt: YES"


p.custom = p.dat.many %>%
  filter(!(type=="treatment" & dist.nonhigh =="> 120 m"),
         mgmt.factorial.nofire %in% c(a,b,c,d,e,f,g))

p.custom$custom.cat = NA

p.custom[p.custom$mgmt.factorial.nofire == a & p.custom$type=="internal",]$custom.cat = "a"
p.custom[p.custom$mgmt.factorial.nofire == b & p.custom$yr.pltd == 2,]$custom.cat = "b"
p.custom[p.custom$mgmt.factorial.nofire == c & p.custom$yr.pltd %in% c("3","4+"),]$custom.cat = "c"
p.custom[(p.custom$mgmt.factorial.nofire == d)  & (p.custom$yr.pltd %in% c(3)) & (p.custom$type == "internal"),]$custom.cat = "d"
p.custom[p.custom$mgmt.factorial.nofire == e  & p.custom$yr.pltd %in% c(2,3) & p.custom$type == "treatment",]$custom.cat = "e"
p.custom[p.custom$mgmt.factorial.nofire == f,]$custom.cat = "f"
p.custom[p.custom$mgmt.factorial.nofire == g,]$custom.cat = "g"

p.custom = p.custom %>%
  filter(!is.na(custom.cat))


# need to restore the control plots back to the filtered candidate plots
p.ctl <- p[p$type=="control",]
p.ctl.match <- p.ctl[p.ctl$id %in% p.custom$ctl.id,]
newcols <- setdiff(names(p.custom),names(p.ctl.match)) # which columns were added to the paired plots?
p.ctl.match[,newcols] <- NA # add them
p.custom <- p.custom[,names(p.ctl.match)] # order cols the same
p.ctl.match <- p.ctl.match[,names(p.custom)]
p.custom.w.ctl <- rbind(p.custom,p.ctl.match)

# write the filtered candidate plot dataset
st_write(p.custom.w.ctl,"data/site-selection/output/candidate-plots/candidate_plots_paired_filtered_moontelope_45m-45m_cottonwood_customfilter.gpkg",delete_dsn=TRUE)

## write internal and perimeter separately
int = p.custom.w.ctl %>%
  filter(type=="internal")
perim = p.custom.w.ctl %>%
  filter(type=="treatment")
ctl = p.custom.w.ctl %>%
  filter(type=="control")

# write the filtered candidate plot dataset
st_write(int,"data/site-selection/output/candidate-plots/candidate_plots_paired_filtered_cottonwood_customfilter_internal.gpkg",delete_dsn=TRUE)
st_write(perim,"data/site-selection/output/candidate-plots/candidate_plots_paired_filtered_cottonwood_customfilter_perimeter.gpkg",delete_dsn=TRUE)
st_write(ctl,"data/site-selection/output/candidate-plots/candidate_plots_paired_filtered_cottonwood_customfilter_control.gpkg",delete_dsn=TRUE)



## Plot environmental range of each factorial management type
mgmt.cats <- unique(p.custom$mgmt.factorial)
fires <- unique(p.custom$fire.dist2)
...
pdf("data/site-selection/output/candidate-plots/stratification_v24_cottonwood_45m-45m_customfilter.pdf")
for(i in seq_along(plts)) {
  print(plts[[i]])
}
dev.off()
