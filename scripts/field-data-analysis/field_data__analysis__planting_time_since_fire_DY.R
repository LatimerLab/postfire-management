...
newdat = data.frame(tpi2000 = mean(plot_dhm$tpi2000),
                    elev = mean(plot_dhm$elev),
                    Shrubs = mean(plot_dhm$Shrubs),
                    facts.planting.first.year = 2,
                    fsplanted = "planted",
                    tmean = mean(plot_dhm$tmean),
                    normal_annual_precip= seq(min(plot_dhm$normal_annual_precip),max(plot_dhm$normal_annual_precip),length.out=10),
                    log10SeedWallConifer = mean(plot_dhm$log10SeedWallConifer),
                    LitDuff = mean(plot_dhm$LitDuff),
                    Fire = plot_dhm$Fire[1],
                    PairID = "E0072")

pred_planted = predict(pltd,newdat, re.form = NA)
newdat$fsplanted = "unplanted"
pred_unplanted = predict(pltd,newdat, re.form = NA)

planting_increase = (pred_planted - pred_unplanted) * pred_unplanted
