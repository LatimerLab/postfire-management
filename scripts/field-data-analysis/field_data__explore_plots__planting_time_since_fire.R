...
cor(plots %>% filter(Fire == "AmRiv") %>% dplyr::select(elev, rad_spring_summer, slope_dem, normal_annual_precip),  use = "complete.obs", method = "pearson")

##### Create plots with each fire in on panel -------------------------------------------------------------------------
...
ggplot(plot_dhm_long %>% filter(metric %in% c("dens.planted", "dens.conif")), 
       aes(x = rad_spring_summer, y =log(value+ 1), shape = Fire, color =Fire)) +
